Feature: Posting to a personal timeline
  ...
  Scenario: Alice can publish a message to a personal timeline
    ...
    Then the message along with the current timestamp is added to Alice's posting archive
  ...
  Scenario: Bob can publish multiple messages to a personal timeline
    ...
    Then the messages along with the current timestamp is added to Bob's posting archive